...
class SimpleTransformerIntentClassifier(nn.Module):
    ...
    def __init__(
        ...
        frame_input_dim = pose_dim * (2 if use_velocity_features else 1)
    ...
    def forward(self, pose: torch.Tensor, lengths: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Args:
            pose: (B, T, pose_dim) 3D pose sequence padded with zeros
            lengths: optional tensor of shape (B,) with true sequence lengths
        Returns:
            (B, num_classes) intent logits
        """
        B, T, _ = pose.shape
        device = pose.device
        ...
        if lengths is not None:
            ...
            if lengths.ndim > 1:
                lengths = lengths.view(B)
            time_index = torch.arange(T, device=device).unsqueeze(0).expand(B, T)
            valid_frames = time_index < lengths.unsqueeze(1)
        else:
